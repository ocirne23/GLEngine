...
uint getGLTextureType(const ETextureType& type)
{
	...
	case ETextureType::TEXRECT:      return GL_TEXTURE_RECTANGLE;
...
uint getGLSizedInternalFormat(const ETextureComponentFormat& format, uint numComponents)
{
	...
	switch (format)
	{
	case ETextureComponentFormat::F8:       return eastl::array<uint, 4>{ GL_R8, GL_RG8, GL_RGB8, GL_RGBA8 }[numComponents];
	case ETextureComponentFormat::F16:      return eastl::array<uint, 4>{ GL_R16, GL_RG16, GL_RGB16, GL_RGBA16 }[numComponents];
	case ETextureComponentFormat::F32:      return eastl::array<uint, 4>{ GL_R32F, GL_RG32F, GL_RGB32F, GL_RGBA32F }[numComponents];
	case ETextureComponentFormat::F8SNORM:  return eastl::array<uint, 4>{ GL_R8_SNORM, GL_RG8_SNORM, GL_RGB8_SNORM, GL_RGBA8_SNORM }[numComponents];
	case ETextureComponentFormat::F16SNORM: return eastl::array<uint, 4>{ GL_R16_SNORM, GL_RG16_SNORM, GL_RGB16_SNORM, GL_RGBA16_SNORM }[numComponents];
	case ETextureComponentFormat::I8:       return eastl::array<uint, 4>{ GL_R8I, GL_RG8I, GL_RGB8I, GL_RGBA8I }[numComponents];
	case ETextureComponentFormat::I16:		return eastl::array<uint, 4>{ GL_R16I, GL_RG16I, GL_RGB16I, GL_RGBA16I }[numComponents];
	case ETextureComponentFormat::I32:		return eastl::array<uint, 4>{ GL_R32I, GL_RG32I, GL_RGB32I, GL_RGBA32I }[numComponents];
	case ETextureComponentFormat::UI8:		return eastl::array<uint, 4>{ GL_R8UI, GL_RG8UI, GL_RGB8UI, GL_RGBA8UI }[numComponents];
	case ETextureComponentFormat::UI16:		return eastl::array<uint, 4>{ GL_R16UI, GL_RG16UI, GL_RGB16UI, GL_RGBA16UI }[numComponents];
	case ETextureComponentFormat::UI32:		return eastl::array<uint, 4>{ GL_R32UI, GL_RG32UI, GL_RGB32UI, GL_RGBA32UI }[numComponents];
	...
	case ETextureComponentFormat::DXT1:  assert(numComponents == 3); return GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
	case ETextureComponentFormat::DXT1A: assert(numComponents == 4); return GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;
	case ETextureComponentFormat::DXT3:  assert(numComponents == 4); return GL_COMPRESSED_RGBA_S3TC_DXT3_EXT;
	case ETextureComponentFormat::DXT5:  assert(numComponents == 4); return GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
	default: assert(false); return 0;
	}
}
...
bool isCompressed(const ETextureComponentFormat& format)
{
	...
	case ETextureComponentFormat::DXT1: 
	case ETextureComponentFormat::DXT1A:
	case ETextureComponentFormat::DXT3: 
	case ETextureComponentFormat::DXT5:
		return true;
	default:
		return false;
	}
