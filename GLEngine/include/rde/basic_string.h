...
public:
	...
	basic_string& append(const value_type* str, size_type len)
	{
        if( !str || len == 0 || *str == 0 )
			return *this;
	...
	basic_string& append(const value_type ch)
	{
		append(&ch, 1);
		return *this;
	}
	...
	size_type find(const value_type* needle, size_type offset = 0) const
    {       
		if (offset > length())
			return basic_string::npos;

		const value_type* s(c_str() + offset);
