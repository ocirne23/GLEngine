#include "Graphics/Utils/TiledShading.h"
...
void TiledShading::update(const PerspectiveCamera& a_camera, uint a_numLights, const glm::vec4* a_viewspaceLightPositionRangeList)
{
	...
	for (ushort i = 0; i < a_numLights; ++i)
	{
		...
		IBounds2D bounds2D = ClusteredTiledShadingUtils::sphereToScreenSpaceBounds2D(a_camera, lightPosition, radius, m_screenWidth, m_screenHeight);
