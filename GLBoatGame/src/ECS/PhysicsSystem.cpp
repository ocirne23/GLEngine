...
PhysicsSystem::PhysicsSystem(BoatGame& a_boatGame)
	: m_boatGame(a_boatGame), m_semaphore(1)
{
...
{
	m_semaphore.acquire();
	m_createBodyMessageQueue.block();
	for (CreateBodyMessage& message : m_createBodyMessageQueue.getBackingQueue())
	{
	...
	m_createBodyMessageQueue.getBackingQueue().clear();
	m_createBodyMessageQueue.release();

	m_createJointMessageQueue.block();
	for (CreateJointMessage& message : m_createJointMessageQueue.getBackingQueue())
	{
		assert(m_components[message.id.index].body != NULL);
		b2Joint* joint = m_physicsWorld->CreateJoint(message.jointDef.get());
	}
	m_createJointMessageQueue.getBackingQueue().clear();
	m_createJointMessageQueue.release();
